                                  ...
                                  float *noise )
{
    ...
    for ( i = 0; i < self->magnLen; i++ )
    {
        probSpeech = self->speechProb[i];	//语音概率
        probNonSpeech = 1.f - probSpeech;	//非语音概率　
        // Temporary noise update:
        // Use it for speech frames if update value is less than previous.
        noiseUpdateTmp = gammaNoiseTmp * self->noisePrev[i] +
                         ( 1.f - gammaNoiseTmp ) *
                         ( probNonSpeech * magn[i] + probSpeech * self->noisePrev[i] );
        ...
        float *theFilter )
{
    ...
    for ( i = 0; i < self->magnLen; i++ )
    {
        // Previous estimate: based on previous frame with gain filter(增益滤波器).
...
// |mode| = 0 is mild (6dB), |mode| = 1 is medium (10dB) and |mode| = 2 is aggressive (15dB).
                            ...
                            float *const *outFrame )
{
    ...
    energy1 = Energy ( winData, self->anaLen );//保存维纳窗系统与输入数据的乘积

    if ( energy1 == 0.0 )
    {
        // Synthesize the special case of zero input.合成零输入的特殊情况
    ...
    if ( self->gainmap == 1 && self->blockInd > END_STARTUP_LONG )
    {
        factor1 = 1.f;
        factor2 = 1.f;
        energy2 = Energy ( winData, self->anaLen );//输入数据的乘积
        gain = ( float ) sqrt ( energy2 / ( energy1 + 1.f ) );//energy2与energy1的比值

        // Scaling for new version(新版本的缩放比例).
        if ( gain > B_LIM )
        {
